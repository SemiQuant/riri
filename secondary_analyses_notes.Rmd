...
res_2b <- results(dds_2, contrast = c("Kasugamycin","500","0"))
v2 <- ggplotly(
...
Here we are using a likelihood ratio test to see if a change is occuring at the translation level but not transcription level, not sure how to explain that better, but we are fitting a negative binomial generalized log-linear model to the read counts for each gene.
...
# change this to read from the metadata table
gp <- c("NT", "NT", "30", "30")
design <- model.matrix(~0 + as.factor(gp))
colnames(design) <- sub("gp", "", colnames(design))
