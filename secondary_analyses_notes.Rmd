...
res_1 %>% 
  data.frame() %>% 
  filter(padj >= 0.5, abs(log2FoldChange) >= 0.5) %>% 
  nrow()
...
meta <- tibble::tribble(
      ...
      "NC_1_10_CKDL220001075-1A_HH5FFCCX2",    "NC-1",           0L,
      "NC_2_10_CKDL220001076-1A_HH5FFCCX2",    "NC-2",           0L,
  "R30min_1_12_CKDL220001077-1A_HH5FFCCX2", "30min-1",          30L,
  "R30min_2_10_CKDL220001078-1A_HH5FFCCX2", "30min-2",          30L,
     "R6h_1_10_CKDL220001079-1A_HH5FFCCX2",    "6h-1",         500L,
     "R6h_2_10_CKDL220001080-1a_HH5FFCCX2",    "6h-2",         500L
  )
```
...
files_in <- list.files("/Volumes/space_dandy/riboseq_processed/ribSeq", "*_count_metagene_profile.txt",
                       full.names = T, recursive = T)
meta_gene <- files_in %>%
...
bam_path <- list.files("/Volumes/space_dandy/riboseq_processed/ribSeq/", "*.bam$",
                       full.names = T, recursive = T)
