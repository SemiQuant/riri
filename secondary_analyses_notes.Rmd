...
install.packages(c("plotly", "tidyverse", "BiocManager", "DESeq2", "parallel", "xtail", "devtools",
                   "ggplot2", "heatmaply", "adegenet", "DT", "limma"), quietly = T)
...
-   I compared the 2 conditions to the No treatment, but we can do more complex models that are harder to interpret/explain if you like.
...
-   I'd recommend in future doing a minimum of three samples per condition, and ideally four to five.
...
resultsNames(dds_1)
res_1 <- DESeq2::results(dds_1, contrast = c("Kasugamycin","30","0"))
v1 <- ggplotly(
  vol_plot(deSeq_results = res_1, adj_p = 0.05, num_annotate = 10, effect_sizeThresh = 2, makePlot = T)
)

res_1b <- DESeq2::results(dds_1, contrast = c("Kasugamycin","500","0"))
v2 <- ggplotly(
...
files_in <- list.files("/Users/SemiQuant/Bioinformatics/Projects/riri/analysis/metagene", "_metagene_profile.txt",
                       full.names = T, recursive = T)
...
colnames(meta_gene) <- basename(files_in)
meta_gene$x <- c(-50:99)
...
meta_gene$Sample <- gsub("_metagene_profile.txt", "", meta_gene$Sample)

meta_gene <- meta_gene %>% 
  separate(Sample, c("Sample", "genes"), sep = "_count_")

meta_gene$Sample <- substr(meta_gene$Sample, 1, 3)
meta_gene <- meta_gene %>% 
  filter(!is.na(metagene_average))
meta_gene <- meta_gene %>% 
  group_by(x, Sample, genes) %>% 
  mutate(metagene_average = mean(metagene_average)) %>% 
  unique()


meta_gene %>% 
  pivot_wider(names_from = c(Sample, genes),
              values_from = metagene_average
  ) %>% 
  ungroup %>% 
  dplyr::select(-x) %>% 
...
palette <- randomcoloR::distinctColorPalette(40)
#palette <-  RColorBrewer::brewer.pal(100, "Set3")

meta_gene %>% 
  ungroup() %>% 
  plot_ly(x = ~x, y = ~metagene_average, color = ~paste(genes, Sample), type = 'scatter', mode = 'lines', colors = palette) %>% 
  layout(legend = list(orientation = 'h'))
...
resultsNames(dds_2)
res_2 <- DESeq2::results(dds_2, contrast = c("Kasugamycin","30","0"))
v1 <- ggplotly(
  vol_plot(deSeq_results = res_2, adj_p = 0.05, num_annotate = 10, effect_sizeThresh = 2, makePlot = T)
)

res_2b <- DESeq2::results(dds_2, contrast = c("Kasugamycin","500","0"))
v2 <- ggplotly(
...
res_1ba <- data.frame(ID = rownames(res_1b),
                      RNAseq = res_1b$log2FoldChange,
                      threshold_RNA = res_1b$padj <= 0.05
)
...
rna_rib$DifferentDirections <- ifelse((rna_rib$RiboSeq > 0 & rna_rib$RNAseq > 0) | (rna_rib$RiboSeq < 0 & rna_rib$RNAseq < 0),
                                      "No", "Yes")
rna_rib_b$DifferentDirections <- ifelse((rna_rib_b$RiboSeq > 0 & rna_rib_b$RNAseq > 0) | (rna_rib_b$RiboSeq < 0 & rna_rib_b$RNAseq < 0),
                                      "No", "Yes")
...
rna_rib$DifferentDirections_Both <- ifelse(rna_rib$DifferentDirections_30 == "Yes" & rna_rib$DifferentDirections_500 == "Yes", "Yes", "No")
...
Translational Efficiency (TE) is the ratio of normalized ribosome footprint abundance to mRNA density.
I used Fragments Per Kilobase of transcript per Million mapped reads (FPKM) to normalize as this seems to still be the standard for TE.

In this analysis we are testing if there is a linear correlation between the log2 transformed translational efficiency calculated by the ratios of RPFs to mRNA with real translational efficiency.

Here I am including all three in the model at once, N, 30m and 6h. I then use empirical bayes to determine if the model is different. Looking at it now, I think you actually want ot do that part seperatley, as you 30m and 6h are expected to be very similar. I've added that below.

```{r DTE, echo=TRUE, message=FALSE, warning=FALSE}
### Differential translational efficiency

get_counts <- function(mn_fld, ext = "_HTSeq.gene.counts.tsv"){
  
  files_in <- list.files(mn_fld, paste0("*", ext),
                         full.names = T, recursive = T)
  ...
  colnames(counts) <- gsub(ext, "", basename(files_in))
  counts <- cbind(genes, counts)
...
feat <- genFeatures(txdb, featuretype = c("promoter", "cds", "intergenic"), reduce_ranges = TRUE,
                    upstream = 50, downstream = 100, verbose = TRUE)
feat <- data.frame(feat$cds_red)

feat <- data.frame(GeneID = feat$feature_by,
                   Chr = feat$seqnames,
                   Start = feat$start,
                   End = feat$end,
                   Strand = feat$strand,
...
cnts <- list(RPFs = get_counts("/Users/SemiQuant/Bioinformatics/Projects/riri/analysis/ribSeq/", "_HTSeq.gene.counts.tsv"),
             mRNA = get_counts("/Users/SemiQuant/Bioinformatics/Projects/riri/analysis/rnaSeq", ".NC_000962.HTSeq.counts"),
             annotation = feat)


table(rownames(cnts$mRNA)%in%cnts$annotation$GeneID)
cnts$annotation <- cnts$annotation[cnts$annotation$GeneID%in% rownames(cnts$RPFs)& cnts$annotation$GeneID %in% rownames(cnts$mRNA),]


cnts$RPFs <- cnts$RPFs[match(cnts$annotation$GeneID, rownames(cnts$RPFs)), ]
cnts$RPFs[is.na(cnts$RPFs)] <- 0
table(cnts$annotation$GeneID==rownames(cnts$RPFs))
rownames(cnts$RPFs) <- cnts$annotation$GeneID

cnts$mRNA <- cnts$mRNA[match(cnts$annotation$GeneID, rownames(cnts$mRNA)), ]
cnts$mRNA[is.na(cnts$mRNA)] <- 0
table(cnts$annotation$GeneID==rownames(cnts$mRNA))
rownames(cnts$mRNA) <- cnts$annotation$GeneID


cbind(colnames(cnts$RPFs), colnames(cnts$mRNA))
...
# change this to read from the metadata table
gp <- as.factor(c("NT", "NT", "30m", "30m", "6h", "6h"))
...
make_TE_plot <- function(TE = TE, col_in = 1, fit_p = fit2$p.value[,1]){
  fit_p <- round(fit_p, 4)
  xaxis <- "mRNA"
  mRNA <- TE$mRNA
  RPFs <- TE$RPFs
  ...
  p1 <- data.frame(x = x,
                   y = te,
                   Gene = rownames(TE$RPFs)) %>% 
    plot_ly(x = ~x, y=~y, type = 'scatter', mode = 'markers', text = ~Gene, 
            hovertemplate = paste(
              "<b>Gene: </b>%{text}<br>",
              "<b>RNAseq: </b>%{x}<br>",
              "<b>RiboSeq: </b>%{y}<br>",
    ...
    layout(#title = 'Styled Scatter',
      yaxis = list(title =  "Translational Efficiency"),
      xaxis = list(title = "mRNA level"))
  
  p2 <- plot_ly(y = ~te, type = "histogram")
  
  
  return(subplot(p1,p2, shareY = T))
...
# subplot(
#   make_TE_plot(TE = TE, col_in = 1, fit_p = fit2$p.value["NT"]),
#   make_TE_plot(TE = TE, col_in = 2, fit_p = fit2$p.value["NT"]),
#   make_TE_plot(TE = TE, col_in = 3, fit_p = fit2$p.value["30m"]),
#   make_TE_plot(TE = TE, col_in = 4, fit_p = fit2$p.value["30m"]),
#   make_TE_plot(TE = TE, col_in = 5, fit_p = fit2$p.value["6h"]),
#   make_TE_plot(TE = TE, col_in = 6, fit_p = fit2$p.value["6h"]),
#   nrows = 3
# )



fit_p <- data.frame(adj.p = round(fit2$p.value[,1], 4))
fit_p$genes <- rownames(fit_p)
x <- TE$RPFs
x$genes <- rownames(x)
te <- TE$TE
te$genes <- rownames(te)
...
te$name <- substr(te$name, 1, 3)
x$name <- substr(x$name, 1, 3)


x$value <- log2(x$value)
te$value <- log2(te$value)
...
dat <- te %>% 
  left_join(x, by = c("genes", "name")) %>% 
  left_join(fit_p) 

dat %>% 
  plot_ly(x = ~RNA, y=~TE, type = 'scatter', mode = 'markers', text = ~genes, color = ~name,
          hovertemplate = paste(
            "<b>Gene: </b>%{text}<br>",
            "<b>RNAseq: </b>%{x}<br>",
            "<b>TE: </b>%{y}<br>",
            "<b>adj p-value: </b>", dat$adj.p
          )) %>% 
  layout(#title = 'Styled Scatter',
    yaxis = list(title =  "Translational Efficiency"),
    xaxis = list(title = "mRNA level"))
...
#-----------
print("Translation Efficiencies, mean and log transformed")
dat_te <- TE.log2
dat_te$genes <- rownames(dat_te)
dat_te <- dat_te %>% 
  pivot_longer(-genes) %>% 
  group_by(genes, name) %>% 
  mutate(value = mean(value)) %>% 
...
te_1 <-  plot_ly(x=tt, y=xx, type="scatter") %>% 
  add_ribbons(x=ll.df$tt, ymin=ll.df$lb, ymax=ll.df$ub, name="95% CI of loess fit", line=list(opacity=0.4, width=0, color="#366092")) %>% 
  layout(yaxis = list(title = 'NT'),
         xaxis = list(title = "30m"))



# te_1 <- dat_te %>%
  # plot_ly(x = ~`30m`, y = ~NT, text = ~genes)
...
te_2 <-  plot_ly(x=tt, y=xx, type="scatter") %>% 
  add_ribbons(x=ll.df$tt, ymin=ll.df$lb, ymax=ll.df$ub, name="95% CI of loess fit", line=list(opacity=0.4, width=0, color="#366092")) %>% 
  layout(yaxis = list(title = 'NT'),
         xaxis = list(title = "6h"))
...
# te_2 <- dat_te %>%
#   plot_ly(x = ~`6h`, y = ~NT, text = ~genes)



subplot(te_1, te_2, shareY = T)
...
### Use raw counts to test whether the change is coming from translation level but not transcription level.

Here we are using a likelihood ratio test to see if a change is occurring at the translation level but not transcription level, not sure how to explain that better, but we are fitting a negative binomial generalized log-linear model to the read counts for each gene.

```{r DTE2, echo=TRUE, message=FALSE, warning=FALSE}
cnts_30 <- cnts
cnts_30$RPFs <- cnts_30$RPFs[c(1,2,3,4)]
cnts_30$mRNA <- cnts_30$mRNA[c(1,2,3,4)]

# change this to read from the metadata table
gp <- as.factor(c("NT", "NT", "30", "30"))
design <- model.matrix(~0 + gp)
colnames(design) <- sub("gp", "", colnames(design))
y <- lapply(cnts_30[c("RPFs", "mRNA")], function(.cnt){
...
mRNA_zero <- rownames(cnts_30[["mRNA"]])[rowSums(cnts_30[["mRNA"]]) == 0]
RPFs_unique <- RPFs_unique[!RPFs_unique %in% mRNA_zero]
data.frame(results[["RPFs"]][RPFs_unique, ]) %>% 
  datatable(caption = "Difference between No treatment and 30m Kasugamycin",
            extensions = 'Buttons',
            options = list(
              paging = TRUE,
              searching = TRUE,
...
cnts_6 <- cnts
cnts_6$RPFs <- cnts_6$RPFs[c(1,2,5,6)]
cnts_6$mRNA <- cnts_6$mRNA[c(1,2,5,6)]

# change this to read from the metadata table
gp <- as.factor(c("NT", "NT", "6", "6"))
design <- model.matrix(~0 + gp)
colnames(design) <- sub("gp", "", colnames(design))
y <- lapply(cnts_6[c("RPFs", "mRNA")], function(.cnt){
  .y <- DGEList(counts = .cnt,
                samples = data.frame(gp=gp))
  .y <- calcNormFactors(.y)
  .y <- estimateDisp(.y, design)
...
RPFs_unique <- setdiff(rownames(results[["RPFs"]]),
                       rownames(results[["mRNA"]]))
mRNA_zero <- rownames(cnts_6[["mRNA"]])[rowSums(cnts_6[["mRNA"]]) == 0]
RPFs_unique <- RPFs_unique[!RPFs_unique %in% mRNA_zero]
data.frame(results[["RPFs"]][RPFs_unique, ]) %>% 
  datatable(caption = "Difference between No treatment and 6h Kasugamycin",
            extensions = 'Buttons',
            options = list(
              paging = TRUE,
              searching = TRUE,
...
## Codon usage
working, but  why some .1? and can also do a normalization to the numer of codons if needed
```{r codons, echo=TRUE, message=FALSE, warning=FALSE}
require(RiboProfiling)


if (FALSE){
  listInputBam <- c(
    BamFile("/Users/SemiQuant/Downloads/delete/NC_1_10_CKDL220001075-1A_HH5FFCCX2.bam"),
    BamFile("/Users/SemiQuant/Downloads/delete/NC_2_10_CKDL220001076-1A_HH5FFCCX2.bam"),
    BamFile("/Users/SemiQuant/Downloads/delete/R30min_1_12_CKDL220001077-1A_HH5FFCCX2.bam"),
    BamFile("/Users/SemiQuant/Downloads/delete/R30min_2_10_CKDL220001078-1A_HH5FFCCX2.bam"),
    BamFile("/Users/SemiQuant/Downloads/delete/R6h_1_10_CKDL220001079-1A_HH5FFCCX2.bam"),
    BamFile("/Users/SemiQuant/Downloads/delete/R6h_2_10_CKDL220001080-1a_HH5FFCCX2.bam")
  )
  
  gtf_file <- "/Users/SemiQuant/Bioinformatics/Projects/riri/references/NC_000962.gff"
  txdb <- GenomicFeatures::makeTxDbFromGFF(file = gtf_file)
  tmp <- riboSeqFromBAM(listInputBam, txdb=txdb, offsetStartEnd = "end")
  
  all_codons <- NULL
  for (bam in 1:length(tmp)){
    listReadsCodon <- tmp[[bam]]
    listReadsCodon <- listReadsCodon[[2]]
    # names(listReadsCodon)
    
    cds <- GenomicFeatures::cdsBy(txdb, use.names=TRUE)
    
    orfCoord <- cds[names(cds) %in% names(listReadsCodon)]
    genomeSeq <- BSgenome.Mtuberculosis.NCBI.ASM19595v2::Mtuberculosis
    
    #codon frequency, coverage, and annotation
    codonData=NULL
    codonData <- codonInfo(listReadsCodon, genomeSeq, orfCoord, motifSize = 3)
    # head(codonData)
    ...
    codonData$value <- codonData$value/(sum(codonData$value))
    
    codonData$Sample <- names(tmp)[bam]
    
    all_codons <- rbind(all_codons, codonData)
    
  }
  
  save.image("codon_dat.Rdata")
}


load("/Users/SemiQuant/Bioinformatics/Projects/riri/analysis/codon_dat.Rdata")
all_codons %>% 
  plot_ly(x = ~name, y= ~value, color = ~Sample) %>% 
  layout(xaxis = list(categoryorder = "total ascending"),
         legend = list(orientation = 'h'))



all_codons$Sample <- substr(all_codons$Sample, 1, 3)
all_codons$Sample[all_codons$Sample  == "R6h"] <- "T6h"
...
### Differential translational efficiency singles
30m
```{r TE_singles_1, echo=FALSE, message=FALSE, warning=FALSE}
gtf_file <- "/Users/SemiQuant/Bioinformatics/Projects/riri/references/NC_000962.gff"
txdb <- GenomicFeatures::makeTxDbFromGFF(file = gtf_file)
feat <- genFeatures(txdb, featuretype = c("promoter", "cds", "intergenic"), reduce_ranges = TRUE,
                    upstream = 50, downstream = 100, verbose = TRUE)
...
# change this to read from the metadata table
gp <- as.factor(c("NT", "NT", "30m", "30m"))
# gp <- c("NT", "NT", "30m", "30m", "6h", "6h")
TE.log2 <- log2(TE$TE + 1)[c(1:4)]
...
print("volcano of above table [i think]")
volc <- topTable(fit3, number=4000)
volc$EffectSize <- volc[,1]-volc[,2]
volc$threshold <- as.factor(volc$adj.P.Val <= 0.05 & abs(volc$EffectSize) >= 1.2)
effect_sizeThresh_2 <- abs(volc[volc$adj.P.Val <= 0.05,]$EffectSize)
effect_sizeThresh_2 <- effect_sizeThresh_2[order(abs(effect_sizeThresh_2), decreasing = T)][20]
volc$ID <- rownames(volc)
volc$lab <- ifelse(volc$adj.P.Val <= 0.05 & abs(volc$EffectSize) >= effect_sizeThresh_2, as.character(volc$ID), "")


volc_DE_1 <- ggplot2::ggplot(data = volc,
                             ggplot2::aes(x = EffectSize, y = -log10(P.Value),
                                          colour = threshold, text = ID)) +
  ggplot2::geom_point(alpha = 0.4, size = 1.75) +
  ggplot2::xlab("Effect Size") + ggplot2::ylab("-log10 p-value") +
  ggplot2::theme_bw() +
  ...
  ggplot2::theme(panel.grid.major = ggplot2::element_blank(),
                 panel.grid.minor = ggplot2::element_blank()) +
  ggplot2::geom_text(ggplot2::aes(label = volc$lab),
                     hjust = 0, vjust = 0, colour = "dark grey", size = 2, nudge_x = 0.2) +
  ggplot2::geom_hline(yintercept = -log10(max(volc$P.Value[volc$adj.P.Val <= 0.05])), colour = "grey", linetype = "longdash") +
  ggplot2::geom_vline(xintercept = c(-1*1.2, 1.2), colour = "grey",linetype = "longdash")

volc30 <- volc_DE_1
print("values are NT - trt")
volc30
...
6h
```{r TE_singles_2, echo=FALSE, message=FALSE, warning=FALSE}
gtf_file <- "/Users/SemiQuant/Bioinformatics/Projects/riri/references/NC_000962.gff"
txdb <- GenomicFeatures::makeTxDbFromGFF(file = gtf_file)
feat <- genFeatures(txdb, featuretype = c("promoter", "cds", "intergenic"), reduce_ranges = TRUE,
                    upstream = 50, downstream = 100, verbose = TRUE)
...
# change this to read from the metadata table
gp <- as.factor(c("NT", "NT", "6h", "6h"))
TE.log2 <- log2(TE$TE + 1)[c(1,2,5,6)]
...
print("volcano of above table [i think]")
volc <- topTable(fit4, number=4000)
volc$EffectSize <- volc[,1]-volc[,2]
volc$threshold <- as.factor(volc$adj.P.Val <= 0.05 & abs(volc$EffectSize) >= 1.2)
effect_sizeThresh_2 <- abs(volc[volc$adj.P.Val <= 0.05,]$EffectSize)
effect_sizeThresh_2 <- effect_sizeThresh_2[order(abs(effect_sizeThresh_2), decreasing = T)][20]
...
# 
# print("side by side")
# print("values are 30min on left")
# cowplot::plot_grid(volc30, volc6)
...
cnts_3 <- cnts
cnts_3$RPFs <- cnts_3$RPFs[c(1,2,3,4)]
cnts_3$mRNA <- cnts_3$mRNA[c(1,2,3,4)]
...
cnts_3_test.results <- xtail(cnts_3$mRNA, cnts_3$RPFs, c("NT", "NT", "3", "3"))
cnts_6_test.results <- xtail(cnts_6$mRNA, cnts_6$RPFs, c("NT", "NT", "6", "6"))
}else{
  load("analysis/xtrail.Rdata")
}
...
cnts_3_test.results
plotFCs(cnts_3_test.results)
# plotRs(cnts_3_test.results)
#not ploting, fix later

resultsTable(cnts_3_test.results) %>% 
  data.frame() %>% 
  datatable(caption = "Difference between No treatment and 30 Kasugamycin",
            extensions = 'Buttons',
            options = list(
              paging = TRUE,
              searching = TRUE,
...
# volcanoPlot(cnts_3_test.results)
volc <- resultsTable(cnts_3_test.results) %>% 
  data.frame()
volc$EffectSize <- volc$log2FC_TE_final
volc$threshold <- as.factor(volc$pvalue.adjust <= 0.05 & abs(volc$EffectSize) >= 1.2)
effect_sizeThresh_2 <- abs(volc[volc$pvalue.adjust <= 0.05,]$EffectSize)
effect_sizeThresh_2 <- effect_sizeThresh_2[order(abs(effect_sizeThresh_2), decreasing = T)][20]
volc$ID <- rownames(volc)
volc$lab <- ifelse(volc$pvalue.adjust <= 0.05 & abs(volc$EffectSize) >= effect_sizeThresh_2, as.character(volc$ID), "")


ggplot2::ggplot(data = volc,
                             ggplot2::aes(x = EffectSize, y = -log10(pvalue_final),
                                          colour = threshold, text = ID)) +
  ggplot2::geom_point(alpha = 0.4, size = 1.75) +
  ggplot2::xlab("Effect Size") + ggplot2::ylab("-log10 p-value") +
  ggplot2::theme_bw() +
  ...
  ggplot2::geom_text(ggplot2::aes(label = volc$lab),
                     hjust = 0, vjust = 0, colour = "dark grey", size = 2, nudge_x = 0.2) + 
  ggplot2::geom_hline(yintercept = -log10(max(volc$pvalue_final[volc$pvalue.adjust <= 0.05])), colour = "grey", linetype = "longdash") +
  ggplot2::geom_vline(xintercept = c(-1*1.2, 1.2), colour = "grey",linetype = "longdash")
...
cnts_6_test.results
plotFCs(cnts_6_test.results)
# plotRs(cnts_6_test.results)
#not ploting, fix later

resultsTable(cnts_6_test.results) %>% 
  data.frame() %>% 
  datatable(caption = "Difference between No treatment and 30 Kasugamycin",
            extensions = 'Buttons',
            options = list(
...
# volcanoPlot(cnts_6_test.results)
volc <- resultsTable(cnts_6_test.results) %>% 
  data.frame()
volc$EffectSize <- volc$log2FC_TE_final
volc$threshold <- as.factor(volc$pvalue.adjust <= 0.05 & abs(volc$EffectSize) >= 1.2)
effect_sizeThresh_2 <- abs(volc[volc$pvalue.adjust <= 0.05,]$EffectSize)
